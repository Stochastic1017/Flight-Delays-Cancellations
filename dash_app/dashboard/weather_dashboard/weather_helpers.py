...
def create_timeseries_plot(station, year, metric, title_info):
    try:
        ...
        if metric != "HourlySkyConditions":
            ...
            # Map severity levels for sky conditions
            severity_map = {
                'CLR': 1.0,  # Clear sky (lowest severity)
                'FEW': 1.5,
                'SCT': 2.0,
                'BKN': 2.5,
                'OVC': 3.0,  # Overcast (high severity)
                'VV': 3.5    # Obscured sky (highest severity)
            }
            norm = mcolors.Normalize(vmin=1, vmax=3.5)
            cmap = cm.inferno_r  # Using reverse Inferno for better visibility

            # Extract primary sky condition and map it to severity
            filtered_df['PrimaryCondition'] = filtered_df['HourlySkyConditions'].str.split(':').str[0]
            filtered_df['Severity'] = filtered_df['PrimaryCondition'].map(severity_map).round(2)

            if filtered_df['Severity'].isnull().all():
                raise ValueError(f"No valid sky condition data available for {station} in {year}")

            # Create the plot
            fig = go.Figure()
            for i in range(1, len(filtered_df)):
                current_severity = filtered_df.iloc[i - 1]['Severity']
                next_severity = filtered_df.iloc[i]['Severity']
                if pd.notna(current_severity) and pd.notna(next_severity):
                    color = mcolors.to_hex(cmap(norm(current_severity)))
                    fig.add_trace(go.Scatter(
                        x=[filtered_df.iloc[i - 1]['UTC_DATE'], filtered_df.iloc[i]['UTC_DATE']],
                        y=[current_severity, current_severity],
                        mode='lines',
                        line=dict(color=color, width=20),  # Increase line width for better visibility
                        hoverinfo='x+y'
                    ))

            # Customize layout
            fig.update_layout(
                title=f'Sky Conditions for {station} in {year}',
                xaxis_title='Time',
                yaxis_title='Sky Condition Severity',
                yaxis=dict(autorange='reversed', range=[0.5, 4]),  # Set range to amplify vertical differences
                height=fig_height,
                width=fig_width,
                template='plotly_dark',
                plot_bgcolor='rgba(0,0,0,0)',  # Ensure background is transparent for contrast
            )
